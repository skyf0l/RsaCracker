...
struct Args {
    ...
    /// Prime number q (supports wildcards: 0x????C0FFEE, ??741827, 0x...C0FFEE, 0xDEADBEEF..., etc.)
    #[clap(short)]
    ...
    dlog: bool,
    /// Public or private key file(s). (RSA, X509, OPENSSH, PKCS#12, CSR in PEM and DER formats.) Can be specified multiple times for multi-key attacks.
    #[clap(short, long)]
    key: Vec<String>,
...
fn main() -> Result<(), MainError> {
    ...
    let max_keys = args.n.len().max(args.e.len()).max(all_ciphers.len());
    ...
    for i in 1..max_keys {
        ...
        let c = all_ciphers.get(i).cloned();
